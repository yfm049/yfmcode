...
public class SqliteUtils extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		String tvsql="create table tvdate(tid integer,tname varchar(100),httpurl varchar(300),hotlink varchar(100),isflag integer,epg varchar(120),logo varchar(200),mode varchar(10))";
		db.execSQL(groupsql);
		db.execSQL(tvsql);
	}
	public void init(){
	...
	public void savetvdate(SQLiteDatabase db,String tid,String tname,String httpurl,String hotlink,String isflag,String epg,String logo,String mode){
		String iscz="select * from tvdate where tid=?";
		Cursor cursor=db.rawQuery(iscz, new String[]{tid});
		if(!cursor.moveToNext()){
			String sql="insert into tvdate(tid,tname,httpurl,hotlink,isflag,epg,logo,mode) values(?,?,?,?,?,?,?,?)";
			db.execSQL(sql, new Object[]{tid,tname,httpurl,hotlink,isflag,epg,logo,mode});
		}
		cursor.close();
	...
	public List<TvInfo> getAllTvinfo(int gid){
		...
		while(cursor.moveToNext()){
			...
			ti.setMode(cursor.getString(cursor.getColumnIndex("mode")));
			info.add(ti);
		}
		cursor.close();
		db.close();

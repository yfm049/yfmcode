...
public class ReportAsyncTask extends AsyncTask<String, Integer, JSONObject> {

	private progressDialogUtils pdu;
	private Context context;
	private Handler handler;

	public ReportAsyncTask(Context context, Handler handler) {
	...
	protected JSONObject doInBackground(String... params) {
		...
		nvps.add(new BasicNameValuePair("a", "setNj123"));
		nvps.add(new BasicNameValuePair("ac", params[0]));
		nvps.add(new BasicNameValuePair("info", params[1]));
		nvps.add(new BasicNameValuePair("type", params[2]));
		nvps.add(new BasicNameValuePair("tel", params[3]));
		nvps.add(new BasicNameValuePair("qq", params[4]));
		nvps.add(new BasicNameValuePair("password", params[5]));

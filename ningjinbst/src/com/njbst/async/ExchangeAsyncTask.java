...
public class ExchangeAsyncTask extends AsyncTask<String, Integer, JSONObject> {
	...
	public ExchangeAsyncTask(Context context, Handler handler,
			List<ExchangeInfo> lmi, Page page, boolean refresh) {
				...
				if (jo.getBoolean("state")) {
					...
					if (jo.has("listdata")) {
						...
						for (int i = 0; i < array.length(); i++) {
							ExchangeInfo linfo = new ExchangeInfo();
	...
	protected JSONObject doInBackground(String... params) {
		...
		nvps.add(new BasicNameValuePair("a", "getGiftOrder"));

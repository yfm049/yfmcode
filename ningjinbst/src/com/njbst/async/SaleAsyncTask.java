...
public class SaleAsyncTask extends AsyncTask<String, Integer, JSONObject> {
	...
	public SaleAsyncTask(Context context, Handler handler,
			List<SaleInfo> lmi, Page page, boolean refresh) {
		...
		try {
			if (jo != null) {
				...
				if (jo.getBoolean("state")) {
					if (jo.has("topsale")) {
						JSONArray array = jo.getJSONArray("topsale");
						if(array.length()>0){
							SaleInfo top = new SaleInfo();
							for (int i = 0; i < array.length(); i++) {
								SaleInfo linfo = new SaleInfo();
								JSONObject item = array.getJSONObject(i);
								linfo.setProduct(item.getString("product"));
								linfo.setBrand(item.getString("brand"));
								linfo.setZekou(item.getString("zekou"));
								linfo.setDesc(item.getString("desc"));
								linfo.setImageurl(item.getString("url"));
								linfo.setLinkurl(item.getString("linkurl"));
								top.getSaleinfos().add(linfo);
							}
							lmi.add(top);
						}
	...
	protected JSONObject doInBackground(String... params) {
		...
		nvps.add(new BasicNameValuePair("a", "spdz"));

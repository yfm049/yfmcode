...
<script type="text/javascript">
	...
	function showaddDialog() {
		window.open("device/adddevice.jsp", "_self");
	}
	function getitem(id){
		window.open("control/data!GetDevice.action?mapparam.id="+id, "_self");
	}
	function deleteitem(id){
		$.post("control/data!DeleteDevice.action",{"mapparam.id":id},function(msg){
			if(msg.state==0){
				alert(msg.msg);
			}else{
	...
	function UpdateState(id,state){
		$.post("control/data!UpdateDeviceState.action",{"mapparam.id":id,"mapparam.state":state},function(msg){
			if(msg.state==0){
				alert(msg.msg);
			}else{
				window.location.reload(true);

...
<script type="text/javascript">
	...
	function showaddDialog() {
		window.open("user/adduser.jsp", "_self");
	}
	function getUser(id){
		window.open("control/data!GetUser.action?mapparam.id="+id, "_self");
	}
	function deleteuser(id){
		$.post("control/data!DeleteUser.action",{"mapparam.id":id},function(msg){
			if(msg.state==0){
				alert(msg.msg);
			}else{
	...
	function UpdateUserState(id,state){
		$.post("control/data!UpdateUserState.action",{"mapparam.id":id,"mapparam.state":state},function(msg){
			if(msg.state==0){
				alert(msg.msg);
			}else{
				window.location.reload(true);
	...
	function adddevice(id){
		window.open("device/adddevice.jsp?userid="+id, "_self");
	}
</script>
</head>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0"
					...
					<tbody>
						...
						<tr <%=i % 2 == 0 ? "bgcolor=#f2f2f2" : ""%> height="30">
							<td class="left_txt2" style="text-align: center;"><a href="control/data!DeviceList.action?mapparam.name=<%=mo.get("name")%>"><%=mo.get("name")%></a></td>

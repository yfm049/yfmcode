...
import com.iptv.thread.LoginMacThread;
...
public class NetUtils {
	...
	public static void macThread(Context context,Handler handler){
		new LoginMacThread(context, handler).start();
	}
	...
	public static void login(String name, String pass, String mac,
		...
		String url = "live.jsp?active=" + name + "&pass="
				+ ComUtils.md5(pass, null) + "&mac=" + mac+"&id="+ComUtils.getDeviceId();
	...
	public static void appCheckUpdate(Handler handler) {
		AppUpdateThread thread = new AppUpdateThread(handler, "live.xml");
		thread.start();
	}

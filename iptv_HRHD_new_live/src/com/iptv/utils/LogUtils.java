	...
	public static void write(String tag,String msg){
		if(isdebug){
			...
			if(Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)){
				try {
					File wjj=new File(Environment.getExternalStorageDirectory().getAbsolutePath()+"/iptv/log");
					if(!wjj.exists()){
						wjj.mkdirs();
					}
					...
					fw.flush();
					fw.close();
					if(log.length()>1024*1024*10){
						log.renameTo(new File(wjj,sdf.format(new Date())+"log.txt"));
					}
				} catch (Exception e) {

...
public class NetUtils {
	...
	public static void login(String name, String pass, String mac,
			Handler handler, SqliteUtils su) {
		String url = "live.jsp?active=" + name + "&pass="
				+ ComUtils.md5(pass, null) + "&mac=" + mac;
		new LoginThread(url, handler, su).start();
	}
	...
	public static void GetBackProData(String url,Handler handler){
		if(profilmthread!=null){
			profilmthread.setIsrun(false);
		}
		profilmthread=new ProFilmThread(handler,url);
		profilmthread.start();
	}
	private static SeekBarUpdateThread sbuThread=null;
	public static void DoSeekBarUpdate(boolean isrun,Handler handler){
		if(sbuThread!=null){
			if(sbuThread.isIsrun()){
				if(!isrun){
					sbuThread.setIsrun(isrun);
					sbuThread=null;
				}
			}else{
				if(isrun){
					sbuThread=new SeekBarUpdateThread(handler);
					sbuThread.start();
				}
			}
		}else{

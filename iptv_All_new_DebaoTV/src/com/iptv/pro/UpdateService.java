...
public class UpdateService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		NetUtils.appLogoUpdate(handler);
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == AppUpdateThread.appchecksuc) {
				...
				if(info!=null){
					ComUtils.setConfig(UpdateService.this, "logocode", info.getVercode());
				}
				Intent intent=new Intent(logoupdate);
				UpdateService.this.sendBroadcast(intent);
			}else if(msg.what==LogoDownloadThreadListener.downloadfail){
				Toast.makeText(UpdateService.this,R.string.logofail, Toast.LENGTH_SHORT).show();
			}else if(msg.what==LogoUpdateThread.logochecksuc){
				info=(Logoinfo)msg.obj;
				if(info!=null&&info.getVercode()>ComUtils.getConfig(UpdateService.this, "logocode", 0)){
					NetUtils.Downlogo(handler,info.getUrl());
				}
			}
		}

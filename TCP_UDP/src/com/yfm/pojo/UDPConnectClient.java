...
public class UDPConnectClient {
	...
	private NioDatagramConnector ioconnector;
	...
	public UDPConnectClient(Handler handler, IoHandlerAdapter iohandler) {
		this.handler = handler;
		ioconnector = new NioDatagramConnector();
			...
			if (iosession != null && MainActivity.issend) {
				try {
					...
					if("255.255.255.255".equals(ip)){
						ioconnector.broadcast(IoBuffer.wrap(p));
					}else{
						iosession.write(IoBuffer.wrap(p));
					}
			...
			while (dingshi && MainActivity.issend) {
				try {
					if (iosession != null && MainActivity.issend) {
						...
						WriteFuture future = iosession.write(IoBuffer.wrap(p));
						future.await();
					}
					Thread.sleep(dtime);
				} catch (Exception e) {

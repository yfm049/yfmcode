...
public class BackPlayActivity extends Activity {
	...
	private void initplayer() {
		if (player == null) {
			player = new MediaPlayer(this,true);
		...
		public void handleMessage(Message msg) {
			...
			if(msg.what==66){
				if(player!=null&&player.isPlaying()&&!isseek){
					...
					playSeek.setProgress((int)player.getCurrentPosition()/1000);
				}
			}
		}
	...
	class MediaPlayerListenerImpl implements OnPreparedListener,
		...
		public void onPrepared(MediaPlayer player) {
			...
			Log.i("tvinfo", "onVideoSize "+width+" "+height);
			holder.setFixedSize(width, height);
			player.start();
			playSeek.setEnabled(true);
			playSeek.setMax((int)player.getDuration()/1000);

...
public class UpdateService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		AppUpdateThread thread = new AppUpdateThread(hu,handler, "update_ptv.xml");
		thread.start();
		HttpUtils logohu=new HttpUtils(this);
		LogoUpdateThread logothread=new LogoUpdateThread(logohu, handler, "logo.xml");
		logothread.start();
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == 1 && msg.obj != null) {
				...
				Utils.setConfig(UpdateService.this, "logocode", info.getVercode());
			}else if(msg.what==3){
				info=(Logoinfo)msg.obj;
				if(info!=null&&info.getVercode()>Utils.getConfig(UpdateService.this, "logocode", 0)){
	...
	public void DownLoadLogo(){
		if (Environment.getExternalStorageState().equals(
			...
			DownLoadThread dt=new DownLoadThread(info.getUrl(), file, new upZipFileUtils(handler));
			dt.start();
		}
	}

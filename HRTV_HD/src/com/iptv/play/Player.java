...
public class Player {
	...
	public void play() {
		LayoutParams lp = playview.getLayoutParams();
		lp.width=playview.getWidth();
		lp.height=playview.getHeight();
		playview.setLayoutParams(lp);
		surfaceholder.setFixedSize(playview.getWidth(), playview.getHeight());
		LogUtils.write(TAG, "设置 setFixedSize"+playview.getWidth()+" "+playview.getHeight());
		LogUtils.write(TAG, "mediaplayer playstart");
		mediaplayer.start();
		playing=true;
		pause=false;
	}

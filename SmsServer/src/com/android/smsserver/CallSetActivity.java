...
public class CallSetActivity extends Activity {
		...
		public void onClick(View v) {
			...
			try {
				...
				if(kaiguan.getCheckedRadioButtonId()==R.id.qian){
					...
					if(c<0){
						Utils.ShowToast(CallSetActivity.this, "必须输入大于0的正整数");
						return;
					}
				}else{
					upload=false;
	...
	private void SendSmsAction(String action){
		...
		Utils.ShowToast(CallSetActivity.this, "指令已发送");
	}
			...
			try {
				...
				if(msg.what==1&&msg.obj!=null){
					...
					if(jo.getInt("state")==1){
						Utils.ShowToast(CallSetActivity.this, "发送成功");
					}else{
						Utils.ShowToast(CallSetActivity.this, "发送失败,请使用短信方式发送");
					}
					
				}else{
					Utils.ShowToast(CallSetActivity.this, "请求服务器错误");
				}
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
				Utils.ShowToast(CallSetActivity.this, "数据错误");
			}
		}

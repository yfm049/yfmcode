...
import com.android.adapter.LocAdapter;
...
public class LocActivity extends Activity {
	...
	private String clienturl="control/server!DeviceLocList.action";
	...
	class ListenerImpl implements OnHeaderRefreshListener,OnFooterRefreshListener,OnClickListener{
		...
		public void onClick(View arg0) {
			// TODO Auto-generated method stub
			Intent intent=new Intent(LocActivity.this,LocSetActivity.class);
	...
	private Handler handler=new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			try {
				...
				if(msg.what==1){
					...
					if(msg.obj!=null){
						...
						for(int i=0;i<data.length();i++){
							...
							item.setAddr(c.getString("location"));
							item.setLatitude(c.getString("latitude"));
							item.setLongitude(c.getString("longitude"));
							item.setTime(c.getString("ctime"));
							list.add(item);
						}
						adapter.notifyDataSetChanged();
					}else{
						Utils.ShowToast(LocActivity.this, "请求服务器错误");
					}
				}
				if(msg.what==2){
					refresh_container.onFooterRefreshComplete();
				...
				e.printStackTrace();
				Utils.ShowToast(LocActivity.this, "数据错误");
			}
		}

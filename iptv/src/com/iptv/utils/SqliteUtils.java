...
public class SqliteUtils extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		String tvsql="create table tvdate(tid integer,tname varchar(100),httpurl varchar(300),hotlink varchar(100),isflag integer,logo varchar(200))";
		db.execSQL(groupsql);
		db.execSQL(tvsql);
	}
	public void init(){
	...
	public void savetvdate(SQLiteDatabase db,String tid,String tname,String httpurl,String hotlink,String isflag,String logo){
		String iscz="select * from tvdate where tid=?";
		Cursor cursor=db.rawQuery(iscz, new String[]{tid});
		if(!cursor.moveToNext()){
			String sql="insert into tvdate(tid,tname,httpurl,hotlink,isflag,logo) values(?,?,?,?,?,?)";
			db.execSQL(sql, new Object[]{tid,tname,httpurl,hotlink,isflag,logo});
		}
		cursor.close();
